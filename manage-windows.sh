#!/usr/bin/env bash
# ============================================================================
#  Al Mahawir al-Khamsah Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/15-al-mahawir-al-khamsah}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.almahawiralkhamsah}"
AVD_NAME="${AVD_NAME:-al_mahawir_al_khamsah_test}"
...
cmd_test() {
  ...
  local out="/tmp/al-mahawir-al-khamsah-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Al Mahawir al-Khamsah Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
