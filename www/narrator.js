/* ═══════════════════════════════════════════════════════════
   NARRATOR.JS — Audiobook narrator for Al-Mahawir al-Khamsah
...
(function() {
  ...
  function setupMediaSession() {
    ...
    var l = getLang();
    navigator.mediaSession.metadata = new MediaMetadata({ title: l === 'ar' ? 'المحاور الخمسة للقرآن' : l === 'fr' ? 'Les Cinq Axes du Coran' : 'The Five Axes of the Quran', artist: l === 'ar' ? 'الشيخ محمد الغزالي' : 'Sheikh Mohammed al-Ghazali', album: STATE.mode === 'book' ? (l === 'ar' ? 'الكتاب كاملاً' : l === 'fr' ? 'Livre complet' : 'Full Book') : getActiveTabName() });
  ...
  function onSpeedChange(val) { STATE.speed = parseFloat(val); var label = document.getElementById('narratorSpeedLabel'); if (label) label.textContent = STATE.speed + 'x'; localStorage.setItem('mk-narrator-speed', STATE.speed); }
  function onPitchChange(val) { STATE.pitch = parseFloat(val); var label = document.getElementById('narratorPitchLabel'); if (label) label.textContent = STATE.pitch.toFixed(1); localStorage.setItem('mk-narrator-pitch', STATE.pitch); }
  ...
  function onKaraokeToggle(checked) { STATE.karaokeEnabled = checked; localStorage.setItem('mk-narrator-karaoke', checked); }
  function onAutoScrollToggle(checked) { STATE.autoScroll = checked; localStorage.setItem('mk-narrator-autoscroll', checked); }
  function onDuoToggle(checked) { STATE.duoReading = checked; localStorage.setItem('mk-narrator-duo', checked); }
  ...
  function loadSettings() { STATE.speed = parseFloat(localStorage.getItem('mk-narrator-speed')) || 1; STATE.pitch = parseFloat(localStorage.getItem('mk-narrator-pitch')) || 1; STATE.karaokeEnabled = localStorage.getItem('mk-narrator-karaoke') !== 'false'; STATE.autoScroll = localStorage.getItem('mk-narrator-autoscroll') !== 'false'; STATE.duoReading = localStorage.getItem('mk-narrator-duo') === 'true'; }
